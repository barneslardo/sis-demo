...
/**
 * Returns a valid org id_token for ID-JAG hop 1, refreshing via refresh_token when needed.
 */
export async function ensureFreshIdToken(req: Request): Promise<string> {
  ...
  if (!idToken) {
    throw new AppError(
      400,
      "OIDC_REQUIRED",
      "Sign in with Okta so your session includes an id_token for agent delegation."
    );
  }
  ...
  if (!refreshToken) {
    throw new AppError(
      401,
      "ID_TOKEN_EXPIRED",
      "Your Okta sign-in token expired. Sign out, then sign in with Okta again to use Ask SIS."
    );
  }
