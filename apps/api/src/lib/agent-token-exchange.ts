...
export async function exchangeIdJagForAccessToken(idJag: string) {
  ...
  let idJagClaims: Record<string, unknown> = {};
  try {
    idJagClaims = decodeJwt(idJag) as Record<string, unknown>;
    console.log(
      `[agent-exchange] hop2 id-jag aud=${idJagClaims.aud} iss=${idJagClaims.iss} sub=${idJagClaims.sub}`
    );
  } catch {
    /* ignore */
  }
