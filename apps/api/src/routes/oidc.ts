...
import { resolveOidcUserRole } from "@sis/shared";
import { config } from "../config.js";
import { AppError } from "../lib/errors.js";
import { isAgentExchangeEnabled, getDelegatedAccessToken, getIdJag, exchangeIdJagForAccessToken } from "../lib/agent-token-exchange.js";
  ...
  try {
    ...
    if (!role) {
      return loginRedirect(
        res,
        "not_authorized",
        "Your account must be in an assigned SIS group (Enrollment Admins, Enrollment Counselor, Student Affairs, Registrar, or Students)."
      );
    }
    ...
    // Passport 0.7 regenerates the session on login — set idToken after login or it is lost.
    ...
    const defaultPath = role === "admin" ? "/admin/students" : "/profile";
    const dest =
      returnTo && returnTo !== "/" && returnTo.startsWith("/") ? returnTo : defaultPath;
...
oidcRouter.get("/agent-status", (req, res) => {
  ...
  res.json({
    data: {
      ...
      /** Human sign-on OIDC app (SIS Admins) — produces id_token for hop 1. */
      oidcAppClientId: config.oidc.clientId || null,
      /** UD AI agent registration — signs hop 1 and hop 2 (private_key_jwt). */
      agentRegistrationId: config.agent.registrationId || null,
      agentClientId: config.agent.clientId || null,
      /** Custom authorization server issuer (RESOURCE_AS_ISSUER / OKTA_ISSUER). */
...
oidcRouter.get("/delegation-probe", async (req, res, next) => {
  try {
    if (!req.user || req.user.role !== "admin") {
      throw new AppError(403, "FORBIDDEN", "Admin session required");
    }
